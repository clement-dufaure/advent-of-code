...
//	static final int Y_DESTINATION = 10;
	...
	static boolean testerAvancee(int x, int y, int offsetX, int offsetY) {
		...
		Changement changement = Changement.calculerChangement(typeCase[x][y], typeCase[x + offsetX][y + offsetY]);

		// Cas du premier changement : s'il est different de rock/narrow on doit sortir
		// la torche
		if (verifierPasEncoreDeChangementDansTableau(dernierChangementAvant[x][y]) && changement != null) {
			if (changement != Changement.RockNarrow) {
				tempsATester += 7;
			}
		} else if (changement != null && !verifierPresenceDansTableau(changement, dernierChangementAvant[x][y])) {
			tempsATester += 7;
		}
		...
		if (x + offsetX == X_DESTINATION && y + offsetY == Y_DESTINATION) {
			...
			} else if (!verifierPresenceDansTableau(Changement.RockNarrow, dernierChangementAvant[x][y])) {
				tempsATester += 7;
			}
		}
		tempsATester++;
		if (meilleurTemps[x + offsetX][y + offsetY] == null
			...
			if (changement != null) {
				verifierEtAjouterNouveauChangement(changement, dernierChangementAvant[x + offsetX][y + offsetY]);
			} else {
		...
		if (meilleurTemps[x + offsetX][y + offsetY] == null || tempsATester < meilleurTemps[x + offsetX][y + offsetY]) {
	...
	static Changement[][][] dernierChangementAvant = new Changement[MAX_X_TEST + 1][MAX_Y_TEST + 1][3];
	...
	static enum Changement {
		...
		static Changement calculerChangement(Type type1, Type type2) {
			...
			if ((type1 == Type.Rock && type2 == Type.Narrow) || (type1 == Type.Narrow && type2 == Type.Rock)) {
				return RockNarrow;
			}
			if ((type1 == Type.Narrow && type2 == Type.Wet) || (type1 == Type.Wet && type2 == Type.Narrow)) {
				return NarrowWet;
			}
			return null;
		}
